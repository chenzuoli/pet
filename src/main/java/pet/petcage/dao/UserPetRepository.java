...
public interface UserPetRepository extends JpaRepository<UserPet, String> {
    ...
    @Transactional
    @Modifying
    @Query(value = "update user_pet set contact = ?1, pet_type = ?2, variety = ?3, nick_name = ?4, gender = ?5, birthday = ?6, avatar_url = ?7, description = ?8 where id = ?9", nativeQuery = true)
    int updateUserPet(String contact, String pet_type, String variety, String nick_name, String gender, String birthday, String avatar_url, String description, String id);
